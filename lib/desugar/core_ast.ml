...
and desugar_expr (expr : Ast.expr) =
  ...
  | Match { match_expr; loc } -> Match {match_expr = desugar_match_expr match_expr; loc }
...
and desugar_let_expr (let_expr : Ast.let_expr) =
  match let_expr with
  | LetDestruct { let_destruct; body } -> LetDestruct { let_destruct = desugar_let_destruct let_destruct; body = desugar_expr body }
  | LetFun { let_fun; body } -> LetBind { let_bind = desugar_let_fun let_fun; body = desugar_expr body }
and desugar_let_destruct { pattern ; value } = {pattern; value = desugar_expr value}
and desugar_let_fun { name; is_rec; params; value } = { name; is_rec; value = desugar_expr (Lambda { loc = Location.set_ghost @@ Ast.loc_of_expr value; lambda = { params = params; body = value } }) }
and desugar_app { left; right } = { left = desugar_expr left; right = desugar_expr right }
and desugar_lambda { params; body } =
  let params = List.rev params in
  List.fold_left params ~init:({ param = List.hd params; body = desugar_expr body })
  ~f:(fun acc_lambda param -> { param; body = Lambda { loc = Location.dummy; lambda = acc_lambda  }} )
and desugar_match_expr { scrutinee ; cases } = { scrutinee = desugar_expr scrutinee; cases = List.map cases ~f:(fun (pat, body) -> (pat, desugar_expr body))}
and desugar_if_expr { cond; then_; else_ } = { cond = desugar_expr cond; then_ = desugar_expr then_; else_= desugar_expr else_} 
and desugar_unary_op { operator; operand } loc =
  let op_name = map_unary_op operator in
  App { loc = loc; app = { left = Lit { loc = Location.dummy; literal = VarLit op_name }; right = desugar_expr operand}}
and desugar_binary_op { operator; left; right } loc = 
  match operator with
  | And -> If { loc; if_expr = { cond = desugar_expr left; then_ = desugar_expr right; else_ = Lit { loc = Location.dummy; literal = BoolLit false } }}
  | Or -> If { loc; if_expr = { cond = desugar_expr left; then_ = Lit { loc = Location.dummy; literal = BoolLit true }; else_ = desugar_expr right }}
  | _ ->
  let op_name = map_binary_op operator in
  let inner = App { loc = Location.dummy; app =
    { left = Lit { loc = Location.dummy; literal = VarLit op_name }; right = desugar_expr left}}
  in
...
and desugar_literal lit =
  ...
  | BoolLit b -> BoolLit b
  | UnitLit -> UnitLit
  | TupleLit { fst; snd; extra } -> TupleLit { fst = desugar_expr fst; snd = desugar_expr snd; extra = List.map extra ~f:desugar_expr}
  | ConstrLit { name; args } -> ConstrLit { name; args = List.map args ~f:desugar_expr  } 

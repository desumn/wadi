...
and pat = Structural of { structural_pat : structural_pat; loc : Location.t }
...
and unary_operator = Neg
and unary_operation = { operator : unary_operator; operand : expr }
...
and binary_operation = { operator : binary_operator; left : expr; right : expr }
and if_expr = { cond : expr; then_ : expr; else_ : expr }
and match_expr = { scrutinee : expr; cases : (pat * expr) list }
and lambda = { params : pat list; body : expr }
and app = { left : expr; right : expr }
and let_destruct = { pattern : pat; value : expr }
and let_fun = { name : string; is_rec : bool; params : pat list; value : expr }
...
and let_top = LetTopDestruct of let_destruct | LetTopFun of let_fun
